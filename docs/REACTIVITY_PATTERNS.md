...
const validationResults = useSync<IValidationResult[]>(/_ ... _/);
...
{/_ ⚠️ Will always show initial value, never updates _/}
</div>
);
}
...
const signal = useSync<{ count: number; items: string[] }>(/_ ... _/);
...
<div>
{hasValue() && <span>Has value!</span>}
{/_ ✅ Will update when signal changes _/}
</div>
);
}
...
{/_ ✅ Will update when validation changes _/}
</div>
);
}
...
const validationResults = useSync<IValidationResult[]>(/_ ... _/);
const formData = useSync<Record<string, any>>(/_ ... _/);
...
const validationResults = useSync<IValidationResult[]>(/_ ... _/);
const showValidation = useSync<boolean>(/_ ... _/);
